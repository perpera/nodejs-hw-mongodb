...
export const requestResetToken = async (email) => {
  ...
  const resetToken = jwt.sign({sub: user._id, email: user.email}, getEnvVar('JWT_SECRET'),{expiresIn: '5m'});

const resetPasswordTemplatePath = path.join(TEMPLATES_DIR, 'reset-password-email.html');

const templateSource = ((await fs.readFile(resetPasswordTemplatePath, 'utf-8')).toString());
